...
function DashboardApp({ user, onLogout }: { user: AuthUser; onLogout: () => Promise<void> }) {
  ...
  useEffect(() => {
    const refreshUnread = () => { void fetchAlerts().then((items) => { const unique = new Map(items.map((item) => [item.id, item])); unreadAlertIdsRef.current = new Set([...unique.values()].filter((item) => item.unread && !pendingReadIdsRef.current.has(item.id)).map((item) => item.id)); setUnreadAlerts(unreadAlertIdsRef.current.size); }).catch(() => undefined); };
    const syncRead = (event: Event) => { const detail=(event as CustomEvent<{id:string;phase:"pending"|"committed"|"rollback"}>).detail;if(!detail?.id)return;if(detail.phase==="pending"){pendingReadIdsRef.current.add(detail.id);unreadAlertIdsRef.current.delete(detail.id)}else if(detail.phase==="rollback"){pendingReadIdsRef.current.delete(detail.id);unreadAlertIdsRef.current.add(detail.id)}else{pendingReadIdsRef.current.delete(detail.id);unreadAlertIdsRef.current.delete(detail.id)}setUnreadAlerts(unreadAlertIdsRef.current.size); };
    refreshUnread();
    ...
    return () => { window.clearInterval(timer); window.removeEventListener("focus", refreshUnread); window.removeEventListener("antam:alerts-changed", refreshUnread); window.removeEventListener("antam:alert-status", refreshUnread); window.removeEventListener("antam:alert-read", syncRead); };
  }, []);
