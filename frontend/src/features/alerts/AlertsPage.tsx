...
export default function AlertsPage({ canAcknowledge, canResolve }: { canAcknowledge: boolean; canResolve: boolean }) {
  ...
  const updateStatus = (status: AlertEvent["status"], note?: string) => {
    if (!selected) return;
    const needsResolve = ["safe", "false_alarm", "resolved"].includes(status);
    if ((needsResolve && !canResolve) || (!needsResolve && !canAcknowledge)) return;
