...
import { AlertSpeechController, buildAlertSpeechMessage, type AlertSpeechPreferences } from "./alertSpeech";
...
function harness(preferences = enabled) {
  ...
  const synthesis = { speak: vi.fn((utterance: SpeechSynthesisUtterance) => spoken.push({ text: utterance.text, utterance })), cancel: vi.fn(), getVoices: vi.fn(() => []), addEventListener: vi.fn(), removeEventListener: vi.fn() } as unknown as SpeechSynthesis;
